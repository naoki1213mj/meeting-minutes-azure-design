...
- Microsoft Foundry は `Microsoft.CognitiveServices/accounts` kind `AIServices` に `allowProjectManagement=true` を設定し、`Microsoft.CognitiveServices/accounts/projects` child resource `minutes-studio` を作成します。classic `Microsoft.MachineLearningServices/workspaces` hub/project は使いません。
- Runtime APIは引き続きAIServices account-level endpointを使います。Foundry projectはnew Foundry portalでのデモ/実験整理用であり、Speech/OpenAI/CUの既存endpointパスをproject scopedへ変更しません。
...
- 既存public構成から段階的にハードニングする場合は、`enablePrivateArtifacts` と `enableCosmosPrivateEndpoint` を先に有効化し、Function App のPrivate Endpoint到達性を検証してから `lockDownCosmosPublicAccess` を有効化します。到達性未検証のまま一括でCosmos public accessを閉じないでください。
